...
public class DataMunger {
	...
	public String getFileName(String queryString) {
		if (null != queryString && !queryString.isEmpty()) {
			...
			if (fromIndex > -1) {
				return (queryString.toLowerCase().substring(fromIndex + 5)).split(" ")[0];
			}
		}
		return null;
	}
	...
	public String getBaseQuery(String queryString) {
		if (null != queryString && !queryString.isEmpty()) {
			...
			int indexOf = lowerCaseQueryString.indexOf("where");
			if (indexOf < 0) {
				indexOf = lowerCaseQueryString.indexOf("group by");
			}
			if (indexOf < 0) {
				indexOf = lowerCaseQueryString.indexOf("order by");
			}
			if (indexOf > -1) {
				return lowerCaseQueryString.substring(0, indexOf - 1);
			} else {
				return queryString;
			}
		}
